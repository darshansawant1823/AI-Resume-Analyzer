...
export const RecruiterChatView: React.FC<RecruiterChatViewProps> = ({ candidates, jdText, onBack }) => {
  ...
  return (
    <div className="bg-gray-50 min-h-screen animate-scale-in flex flex-col pb-20 sm:pb-0">
      ...
      <div className="flex-1 flex flex-col lg:flex-row max-w-[1400px] mx-auto w-full px-4 sm:px-6 py-8 gap-8 overflow-hidden">
        {/* Main Chat Area */}
        ...
        {/* Sidebar: Candidate Context & JD */}
        <div className="w-full lg:w-80 flex flex-col gap-6">
            <div className="bg-white rounded-3xl p-6 shadow-apple-card border border-white/60">
                <h3 className="text-xs font-bold text-gray-400 uppercase tracking-widest mb-4">Loaded Candidates</h3>
                <div className="space-y-3">
                    {candidates.map((c, i) => (
                        <div key={c.id} className="flex items-center gap-3 p-3 bg-gray-50 rounded-xl border border-gray-100 group hover:bg-white hover:border-system-blue transition-all">
                            <div className="w-8 h-8 bg-white rounded-lg shadow-sm flex items-center justify-center text-xs font-bold text-gray-400 group-hover:text-system-blue transition-colors">
                                #{i + 1}
                            </div>
                            <div className="flex-1 min-w-0">
                                <div className="text-sm font-bold text-gray-900 truncate">{c.name}</div>
                                <div className="text-[10px] font-bold text-system-blue uppercase tracking-tighter">
            ...
            <div className="bg-gray-900 rounded-3xl p-6 shadow-2xl relative overflow-hidden group">
                <div className="absolute top-0 right-0 -mt-8 -mr-8 w-24 h-24 bg-system-blue blur-[40px] opacity-20"></div>
                <h3 className="text-xs font-bold text-white/40 uppercase tracking-widest mb-4 relative z-10">Job Context</h3>
                <div className="relative z-10">
                    <p className="text-sm text-white/80 line-clamp-4 font-serif italic mb-4">
                        {jdText}
                    </p>
                    <button 
                        onClick={() => {
                            const win = window.open('', '_blank');
                            win?.document.write(`<html><head><title>Job Description</title><style>body{font-family:sans-serif;padding:40px;line-height:1.6;max-width:800px;margin:0 auto;}</style></head><body><h1>Job Description</h1><pre style="white-space:pre-wrap;">${jdText}</pre></body></html>`);
                            win?.document.close();
                        }}
                        className="w-full py-3 bg-white/10 hover:bg-white/20 rounded-xl text-xs font-bold text-white transition-all border border-white/10"
                    >
                        View Full JD
                    </button>
                </div>
