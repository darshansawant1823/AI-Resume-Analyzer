    ...
    <div className="space-y-3">
      ...
      <div className="relative group">
        <textarea
          ...
          className="w-full h-48 p-4 bg-gray-50/50 hover:bg-gray-50 border-0 rounded-2xl text-gray-900 placeholder-gray-400 focus:bg-white focus:ring-2 focus:ring-system-blue/20 transition-all duration-200 resize-none text-[15px] leading-relaxed shadow-inner pr-12"
        />
        
        {/* Maximize Button */}
        <button
          onClick={() => setIsMaximized(true)}
          className="absolute top-3 right-3 p-2 text-gray-400 hover:text-system-blue hover:bg-white rounded-lg transition-all opacity-0 group-hover:opacity-100"
          title="Maximize"
        >
        ...
        <div className="fixed inset-0 z-[100] bg-white animate-scale-in flex flex-col">
          <div className="flex items-center justify-between px-6 py-4 border-b border-gray-100 bg-gray-50/50 backdrop-blur-md">
            ...
            <div className="flex items-center gap-4">
               <span className="text-xs font-medium text-gray-400">{value.length} chars</span>
               <button
          ...
          <div className="flex-1 p-6 sm:p-10 max-w-5xl mx-auto w-full">
            <textarea
              ...
              onChange={(e) => onChange(e.target.value)}
              placeholder="Paste the job requirements here..."
              className="w-full h-full resize-none outline-none text-lg leading-relaxed text-gray-800 font-serif placeholder-gray-300"
            />
          </div>
        </div>
      )}
