...
export const CompareProfiles: React.FC<CompareProfilesProps> = ({ candidates, onBack }) => {
  ...
  const getMicroInsight = (c: Candidate) => {
      ...
      if (a.breakdown?.core_skills && a.breakdown.core_skills > 90) return "🛠️ Technical powerhouse";
      if (a.breakdown?.soft_skills && a.breakdown.soft_skills > 90) return "🤝 Great culture add";
  ...
  const handleDownloadPDF = () => {
      ...
      setTimeout(() => {
          ...
          document.title = `Candidate_Comparison_${date}`; // Sets PDF filename
          window.print();
    ...
    <div className="bg-gray-50 min-h-screen animate-scale-in pb-20 print:bg-white print:pb-0 print:min-h-0">
      ...
      <style>{`
        @media print {
          ...
          .print-grid { display: grid !important; grid-template-columns: repeat(${candidates.length}, 1fr) !important; gap: 1rem !important; }
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 py-8 print:p-0 print:max-w-none">
        <div className={`grid grid-cols-1 ${gridCols} gap-6 print-grid`}>
            {candidates.map((c, idx) => {
                ...
                return (
                    <div key={c.id} className={`bg-white rounded-3xl shadow-sm border ${isBaseline ? 'border-system-blue ring-2 ring-blue-100' : 'border-gray-200'} overflow-hidden flex flex-col print-card transition-all duration-300`}>
                        {/* Card Header */}
                        <div className="p-6 border-b border-gray-100 relative bg-gradient-to-b from-white to-gray-50/30">
                            <div className="flex justify-between items-start mb-3">
                                <div>
                                    ...
                                    <h3 className="text-lg font-bold text-gray-900 line-clamp-1" title={c.name}>{c.name}</h3>
                        ...
                        <div className="flex-1 divide-y divide-gray-50">
                            ...
                            {(!showDifferencesOnly || !isRowSimilar(['potential_score'])) && (
                                <div className="p-4 grid grid-cols-2 gap-4 animate-[fadeIn_0.3s_ease-out]">
                                    ...
                                    <div>
                                        ...
                                        <span className="text-sm font-medium text-gray-700 line-clamp-2" title={analysis?.training_estimate}>{analysis?.training_estimate || 'N/A'}</span>
                                    </div>
                                </div>
                            )}
                            ...
                            {analysis?.breakdown && (
                                <div className="p-4 space-y-3">
                                    ...
                                    {[
                                        { label: 'Core Skills', key: 'core_skills' },
                                        { label: 'Title Alignment', key: 'title_alignment' },
                                        { label: 'Experience', key: 'experience_relevance' },
                                        { label: 'Soft Skills', key: 'soft_skills' },
                                        { label: 'ATS Readiness', key: 'ats_readiness' },
                                    ].map(metric => {
                                        // @ts-ignore
                                        const val = analysis.breakdown[metric.key] as number;
                                        const delta = showDelta ? getScoreDelta(val, ['breakdown', metric.key]) : null;
